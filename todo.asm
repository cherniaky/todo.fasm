...
load_todos:
    ...
    ;; check size of file is divisible by TODO_SIZE
    mov rcx, TODO_SIZE
    div rcx
    cmp rdx, 0
    jne .error
    ...
    mov rax, [rsp]
    cmp rax, TODO_SIZE*TODO_CAP
    jle .skip_truncate
    mov qword [rsp], TODO_SIZE*TODO_CAP
.skip_truncate:
    ;; read entire db from file
...
save_todos:
    open todo_db_file_path, O_WRONLY or O_CREAT or O_TRUNC, 420
    cmp rax, 0
    jl .fail
    push rax
...
shutdown_response db "HTTP/1.1 200 OK", 13, 10
                  ...
                  db "Connection: close", 13, 10
                  db 13, 10
                  db "<h1>Shutting down...</h1>", 10
                  db "Please close this tab", 10
                  db 0
